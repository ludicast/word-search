...
var require_diacritics = __commonJS((exports) => {
...
__export(exports_utils, {
  ...
  getWordStartBoundaries: () => getWordStartBoundaries,
  getRandomLetter: () => getRandomLetter,
  getAllCharSequencesFromGrid: () => getAllCharSequencesFromGrid,
  findPathInGrid: () => findPathInGrid,
  filterWordsInGrid: () => filterWordsInGrid,
  fillGrid: () => fillGrid,
  createPathFromPair: () => createPathFromPair,
...
function getAllCharSequencesFromGrid(grid) {
  ...
  for (let y = 0;y < grid.length; y++) {
    sequences.push(grid[y].join(""), readPathFromGrid(0, y, "SE", Math.min(grid.length - y, grid[0].length), grid), readPathFromGrid(0, y, "NE", Math.min(y + 1, grid[0].length), grid));
  }
  for (let x = 0;x < grid[0].length; x++) {
    sequences.push(grid.map((row) => row[x]).join(""));
    if (x > 0) {
      sequences.push(readPathFromGrid(x, 0, "SE", Math.min(grid[0].length - x, grid.length), grid), readPathFromGrid(x, grid.length - 1, "NE", Math.min(grid[0].length - x, grid.length), grid));
    }
  }
  return sequences.filter((x) => x.length > 1).join("|");
}
var createPath = (x, y, dir, len) => {
  return import_lodash.fill(Array(len - 1), 0).reduce((path) => {
...
var createPathFromPair = (start, end) => {
...
var getWordStartBoundaries = (wordLength, direction, cols, rows) => {
  const res = {
    minX: 0,
    maxX: cols - 1,
    minY: 0,
  ...
  if ([res.minX, res.maxX].some((v) => !import_lodash.inRange(v, 0, cols + 1)) || [res.minY, res.maxY].some((v) => !import_lodash.inRange(v, 0, rows + 1))) {
    badInput = true;
  }
  return badInput ? null : res;
};
var normalizeWord = (word, upperCase = true, keepDiacritics = false) => {
...
class WordSearch {
  ...
  forbiddenWordsFound;
  constructor(options = {}) {
    this.settings = import_lodash2.merge(import_lodash2.cloneDeep(defaultSettings), import_lodash2.cloneDeep(options));
    this.settings.allowedDirections = import_lodash2.difference(this.settings.allowedDirections, this.settings.disabledDirections);
    this.forbiddenWordsFound = [];
    this.data = this.buildGame();
  }
  get grid() {
    return import_lodash2.cloneDeep(this.data.grid);
  }
  get words() {
    return import_lodash2.cloneDeep(this.data.words);
  }
  get cleanForbiddenWords() {
    return import_lodash2.cloneDeep(this.settings.forbiddenWords).map((w) => this.cleanWord(w));
  }
  get forbiddenWordsIncluded() {
    return import_lodash2.cloneDeep(this.forbiddenWordsFound);
  }
  get defaultSettings() {
    return import_lodash2.cloneDeep(defaultSettings);
  }
  get utils() {
  ...
  cleanWord(word) {
    return normalizeWord(word, this.settings.upperCase, this.settings.diacritics);
  }
  dump() {
    return import_lodash2.cloneDeep({ settings: this.settings, data: this.data });
  }
  load(config) {
    import_lodash2.merge(this, config);
    return this;
  }
  read(start, end) {
    const path = createPathFromPair(start, end);
    if (path) {
